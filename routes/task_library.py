# Task Library Module
# ===================
# Contains task suggestion library organized by fitness level and category.
# Provides personalized task suggestions based on user profile data.

from datetime import datetime
import random

# Task library organized by fitness level and category
TASK_LIBRARY = {
    'beginner_cardio': [
...
def get_task_suggestions(db, user_id, existing_task_names=None):
    ...
    #     existing_task_names: List of task names user already has (to filter out)
    #
    # Returns:
    #     List of suggested tasks with name and description
    ...
    # Determine fitness level from activity level
    ...
    # Adjust fitness level based on BMI and age for safety
    if bmi > 30:
        fitness_level = 'beginner'
    ...
    # Build suggestions: cardio + strength + nutrition + recovery/lifestyle
